...
<?php session_start(); //on start la session pour recuper les variables
//on include les class et les fonciions
include('include/menu.php');
include('include/database.php');
include('include/gestion.php');
global $db;
if (!isset($_SESSION['user'])) { // verifie si une session(user) existe ou pas si non un lien vers l'accueil apparé si oui le menu ainsi que d'autres informations
    echo "<a href='index.php'>Vous n'etes pas connecter</a>";
} else {  
    menu(); //permet d'aficher le menu

    $selectbateau = $db->prepare("SELECT * FROM `Bateau`"); //requete qui selectionne tout les bateaux est qui va servir a faire un liste deroulante 
    ...
    if (isset($_GET["bateau"])) {
        foreach ($bateau as $objetBeateau) {
            if ($objetBeateau->getId() == $_GET["bateau"]) {
                ...
                //requete en jointure qui permet de selectionner des information grace a id du bateau qui est recupere avec la liste deroulant des bateaux
            }
        }
    }
        ...
        <div class="wrapper row justify-content-center">
            <form action="" methode="GET">
                ...
                <select name="bateau">
                    ...
                    foreach ($bateau as $objetBeateau) { //envoi les informations du tableau bateau a objetbateau là on travaille en objet
                    ...
                    <div class="wrapper fadeInDown">
                        <div id="formContent">
                            <p class="card-text">
                                <table class="table">
                                    <tr>
                                        <th scope="col" class="text-center">Coordonnées GPS N° <?php echo $selectGPSd['GPSID']; //affiche l'id du gps
                                                                                                ?></th>
                                    </tr>
                                    <tr>
